...
from libs.simulation.afm_calculations import amp_phase, e_diss, v_ts
...
def sfs_genmaxwell_lr(N, G, tau, R, dt, simultime, y_dot, y_t_initial, k_m1, fo1, Ge = 0.0, Q1=100, printstep = 1, H = 2.0e-19, Q2=200, Q3=300, startprint = 1, vdw = 1):
    ...
    eta = tau*G
    Gg = Ge
    for i in range(N): #this loop looks silly but if you replace it with Gg = Ge + sum(G[:]) it will conflict with numba making the simulation very slow
        Gg = Gg + G[i]
    t_a = []
    Fts_a = []
    xb_a = []
    ...
    xb = 0.0
    pb = 0.0
    pc, pc_rate = np.zeros(N), np.zeros(N)
